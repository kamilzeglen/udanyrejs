...
export async function extractRawOfferPage(page: Page): Promise<RawOfferPage> {
  return page.evaluate(() => {
    ...
    const shipNameText = document.querySelector('a.info__shipowner-name')?.textContent?.trim() ?? '';

    const companyHref = document.querySelector('a.info__shipowner-logo')?.getAttribute('href') ?? '';
    ...
    const pdfAnchor = Array.from(document.querySelectorAll('a')).find((a) => a.href.includes('offerPdf'));
    const pdfHref = pdfAnchor ? pdfAnchor.href : '';

    const itineraryRows = Array.from(document.querySelectorAll('table.route-table tbody tr')).map((row) => {
      ...
      return {
        ...
        dateText: cells[1]?.querySelectorAll('div')[1]?.textContent?.trim() ?? '',
        cityText,
        arrivalText: cells[3]?.querySelector('span')?.textContent?.trim() ?? '',
        departureText: cells[4]?.querySelector('span')?.textContent?.trim() ?? '',
      };
    });

    const cabinGroupRows = Array.from(document.querySelectorAll('table[aria-describedby="legend"] tr.group')).map(
      (row) => {
        const cells = row.querySelectorAll('td');
        return {
          labelText: cells[0]?.textContent?.trim() ?? '',
          minPriceText: cells[1]?.querySelector('.group-min-price')?.textContent?.trim() ?? '',
        };
      },
    );

    const otherTermLinks = Array.from(document.querySelectorAll('a.schedules__element')).map((link) => {
      const spans = link.querySelectorAll('.schedules__term span');
      const stripWhitespace = (value: string | undefined | null): string => (value ?? '').replace(/\s+/g, '').trim();
    ...
    return { titleText, shipNameText, companyHrefSlug, ogImageContent, pdfHref, itineraryRows, cabinGroupRows, otherTermLinks };
  });
}

export async function extractRawPriceCheckPage(page: Page): Promise<RawPriceCheckPage> {
  const cabinGroupRows = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('table[aria-describedby="legend"] tr.group')).map((row) => {
      const cells = row.querySelectorAll('td');
      return {
        labelText: cells[0]?.textContent?.trim() ?? '',
        minPriceText: cells[1]?.querySelector('.group-min-price')?.textContent?.trim() ?? '',
