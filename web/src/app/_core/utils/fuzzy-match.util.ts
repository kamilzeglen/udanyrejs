...
export function findBestMatch(query: string, candidates: FuzzyMatchCandidate[]): string | null {
  ...
  const exactMatch = candidates.find((candidate) => candidate.name.trim().toLowerCase() === normalizedQuery);
  if (exactMatch) {
